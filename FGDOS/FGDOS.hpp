...
private:
     ...
     */
    [[nodiscard]] inline float getSecondsPassed() const {
        return (static_cast<float>(xTaskGetTickCount() - lastReadTick)) / configTICK_RATE_HZ;
    }
    ...
    /**
     *Only for testing
     *Prints all values from data and coWnfig registers, does not update cached data
     */
